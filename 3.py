...
def kvartira(nomer):
    ...
    for i in range(m):
        while nomer not in c[i]:  # Находим, какому массиву принадлежит число, возвращаем его
            i += 1
            check = i
    print(f'Эта квартира по счету {check + 1} на этаже')
"""
def kvartira(nomer):
    print(f'Квартира находится на {math.ceil(nomer / 4)} этаже')
    print(f'Квартира по счету {abs(nomer - (math.ceil(nomer / 4) - 1) * 4)} на этаже')
...
def kv_pod(nomer):
    ...
    print(f'Данная квартира по счету {abs(nomer - (math.ceil(nomer / 6) - 1) * 6)} на этаже')
